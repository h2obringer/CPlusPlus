...
int MyQueue::pop() {
    if (inOrder.empty()) {
        while (!notInOrder.empty()) {
            inOrder.push(notInOrder.top());
            notInOrder.pop();
        }
    }

    int result = inOrder.top();
    inOrder.pop();
    return result;
}
...
bool MyQueue::empty() {
    return inOrder.empty() && notInOrder.empty();
}
