...
int main(int argc,char **argv){
  ...
  for(i=0;i<argc;++i){
    ...
    /*UNRECOGNIZED ARGUMENTS*/
      ...
      mysshdHelp(); /*mysshell print help function*/
      printf("Arguments given were:\n");
      for (j=1; j < argc; ++j) {
        printf("argv[%d]: '%s'\n", i, argv[i]);
      }
...
static int handleCommands(int fd,char* pword){
  ...
  char *pch; /*used in finding a position on a string*/
  ...
  else if( cc != 0 ) /*if a command was read in*/
  {
    ...
    for(j=0;buf[j]!=NULL;++j){
      if(buf[j]=='\r'){/*get rid of the return carrage and newline characters*/
        buf[j]='\00';
        break;
      }
    }
