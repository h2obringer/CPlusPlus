...
class MonkeyBusiness{
  public:
    MonkeyBusiness();
    MonkeyBusiness(Tree_Node *root);
    ~MonkeyBusiness();
    int MonkeysOnLeaves();
    int MonkeysInTree();
    int HungryMonkeys();  
    double PercentMonkeys();
  private:
    Tree_Node *root_ptr;
...
MonkeyBusiness::MonkeyBusiness(){ //constructor
  root_ptr=NULL;
}
...
MonkeyBusiness::~MonkeyBusiness(){//destructor - calls clear_tree which deletes nodes recursively
  clear_tree(root_ptr);
}
