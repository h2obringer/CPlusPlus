// File: game.cxx
//********************************************************
//Randy Obringer::Changed the play function to make two 
//human moves for a two player game instead of human
//vrs. computer play
//********************************************************
...
#include "colors.h"   // Provides colored text
...
string game::get_user_move( ) const{ //asks user to cin their move
  string answer;
...
/*void game::make_human_move( ){
  ...
  if(!jump_available()){
    ...
    while(jump_available()){
      ...
      move=get_user_move();
      while(move1!=move){
        display_message(BOLD+"You must take the jump!"+RESET+"\n");
        move=get_user_move( );
      }
    }
    make_move(move);
