...
class EscapeTheMaze{
	public:
		int lengthShortestExit(vector<string> &maze, int x, int y); //return the number of steps to the shortest exit
		vector<string> shortestExit(vector<string> &maze, int x, int y);
		
		static const size_t NMAX = 100; //max number of rows and columns in the arrays below
		static const int BIG_VALUE = 1000000; //used as a weight in the graph
