...
//   The parser code was edited by myself (Randal Mark Obringer) to handle appending to a 
...
//
// NOTE: Currently only tested with Linux operating system.
//
// TODO: Possible get to work with both OS's
//
//********************************************************************
...
int main(int argc, char *argv[]){
	int i=ZERO; //used for the for loop
	...
	//      break;
		...
		if(checkIfBuiltIn(data_command)==ONE){
			...
			if(fork()==0){//child (process2)***************************
				if((data_command->numcommands==1)&&(data_command->infile!=NULL)){
				...
				if((data_command->numcommands==1)&&(data_command->outfile!=NULL)){
				...
				if(data_command->numcommands==2){
					if(close(pipefd[READ])!=ZERO){
						printf("Failed to close pipe read end\n");
					}
					if(dup2(pipefd[WRITE],STDOUT_FILENO)==NEG_ONE){
						printf("Failed to redirect output to pipe\n");
					}
					if(close(pipefd[WRITE])!=0){
						printf("Failed to close pipe write end\n");
					}
				}
				if(execvp(data_command->TheCommands[0].command,data_command->TheCommands[0].args)==NEG_ONE){
					printf("Command didn't work\n");
				}
			}else{ //parent (process1) running terminal**********************************
				in_fd=dup(restore_in_fd);
				out_fd=dup(restore_out_fd);
				...
				if(data_command->numcommands==2){
					if(fork()==0){ //child (process3)
						...
						if(close(pipefd[WRITE])!=0){
							printf("Failed to close write end of pipe");
						}
						if(dup2(pipefd[READ],STDIN_FILENO)==NEG_ONE){
							printf("Failed to redirect input to pipe\n");
						}
						if(close(pipefd[READ])!=0){
							printf("Failed to close read end of pipe");
						}
						if(execvp(data_command->TheCommands[1].command,data_command->TheCommands[1].args)==NEG_ONE){
							printf("Command didn't work\n");
						}
					}
				}
				
				//make all other processes wait/sleep
				for(i=0;i<data_command->numcommands;i++){
					ret_pid=wait(&status);
				}
			} //end parent (process1) running terminal
